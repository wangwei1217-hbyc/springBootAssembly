...
SERVER_NAME=`sed '/run.application.name/!d;s/.*://' conf/application-dev.yml | tr -d '\r'`
SERVER_PORT=`sed '/run.server.port/!d;s/.*://' conf/application-dev.yml | tr -d '\r'`
...
#打印控制台日志
nohup java $JAVA_OPTS $JAVA_MEM_OPTS $JAVA_DEBUG_OPTS $JAVA_JMX_OPTS -classpath $CONF_DIR:$LIB_JARS com.wangwei.Application > $STDOUT_FILE 2>&1 &
#不打印控制台日志
